...
def record(conn, *, actor_id=None, action: str, entity: str, entity_id=None, details: dict | None = None) -> None:
    """Append an operational audit event; callers keep it in their transaction."""
    conn.execute(
        ...
        (actor_id, action, entity, str(entity_id) if entity_id is not None else None,
         json.dumps(details or {}, ensure_ascii=False, default=str), datetime.now(timezone.utc).isoformat()),
    )
