(() => {
  ...
  const state = { pending: false, busy: false };
  ...
  function mount() {
    ...
    const send = panel.querySelector('.la-send');
    ...
    panel.querySelector('form').onsubmit = async e => { e.preventDefault(); const text=input.value.trim(); if(!text || state.busy)return; input.value=''; addMessage(text,'user'); await handle(text); };
    ...
    function addMessage(text, who='bot', actions=[]) {
      const div=document.createElement('div'); div.className=`la-msg ${who}`; div.textContent=text;
    ...
    async function handle(text){
      ...
      try {
        ...
        if(result.status==='executed' || result.status==='cancelled' || result.status==='already_processed') state.pending=false;
        if(result.status==='confirmation_required') state.pending=true;
        const actions = result.status==='confirmation_required' ? [
      ...
      finally { state.busy=false; input.disabled=false; send.disabled=false; input.focus(); }
    }
    ...
    function renderResult(result, actions=[]) {
      ...
      if(result.id) text += `\nID: ${result.id}`;
      const delivery=result.invoice_delivery?.whatsapp;
      if(delivery?.status) text += `\nWhatsApp: ${delivery.status}${delivery.error ? ` — ${delivery.error}` : ''}`;
